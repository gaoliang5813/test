...
static int open_input_file(InputFile *input_file)
{
    ...
    for (i = 1; i < global_ctx->video_num; i++)
    {
        *(ifmt_ctx + i) = NULL;
                       ...
                       AVCodecContext *enc_ctx, const char *filter_spec)
{
    ...
    int ret = 0;
    int i;
    const AVFilter **buffersrc = (const AVFilter**)av_malloc(global_ctx->video_num*sizeof(AVFilter*));
...
static int init_spec_filter(int flag)
{
    ...
    for (i = 0; i < stream_num; i++) {
        ...
        if (ifmt_ctx[0]->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            if (global_ctx->grid_num == 1)
                snprintf(filter_spec, sizeof(filter_spec), "color=c=black@1:s=1920x1080[x0];[in0]null[ine0];[ine0]scale=w=960:h=540[inn0];[x0][inn0]overlay=1920*0/2+480:1080*0/2[x1];[x1]null[out]");
            else
            {
                ...
                for (j = 0; j < global_ctx->video_num; j++)
                {
                    ...
                    if(flag == 4) {
                        snprintf(spec_temp, sizeof(spec_temp),
                                  ...
                                  global_ctx->enc_width, x_coor, stride, global_ctx->enc_height, y_coor, stride, k);
                    }else if(flag == 2) {
                        snprintf(spec_temp, sizeof(spec_temp),
                                  "[ine%d]scale=w=%d:h=%d[inn%d];[x%d][inn%d]overlay=%d*%d/%d:%d*%d/%d+%d[x%d];", j,
                                  global_ctx->enc_width / stride, global_ctx->enc_height / stride, j, k - 1, j,
                                  global_ctx->enc_width, x_coor, stride, global_ctx->enc_height, y_coor, stride, global_ctx->enc_height/4, k);
                    }else if(flag == 3) {
                        snprintf(spec_temp, sizeof(spec_temp),
                                  "[ine%d]scale=w=%d:h=%d[inn%d];[x%d][inn%d]overlay=%d*%d/%d+%d:%d*%d/%d[x%d];", j,
                                  global_ctx->enc_width / stride, global_ctx->enc_height / stride, j, k - 1, j,
...
int videocombine(GlobalContext* video_ctx,int flag)
{
    int ret;
    int tmp = 0;
    int got_frame_num = 0;
...
int mult_video_4()
{
    ...
    global_ctx->outfilename = "combined_4.flv";
    global_ctx->input_file = inputfiles;

    return videocombine(global_ctx,4);
}

int mult_video_left_right()
{
    //test 2x2
    ...
    global_ctx->enc_height = 1080;
    global_ctx->enc_width = 1920;
    global_ctx->outfilename = "combined_left_right.flv";
    global_ctx->input_file = inputfiles;

    return videocombine(global_ctx,2);
}

int mult_video_up_down()
{
    //test 2x2
    inputfiles = (InputFile*)av_malloc_array(2, sizeof(InputFile));
    if (!inputfiles)
    ...
    global_ctx->enc_height = 1080;
    global_ctx->enc_width = 1920;
    global_ctx->outfilename = "combined_up_down.flv";
    global_ctx->input_file = inputfiles;
